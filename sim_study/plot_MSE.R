  ...
  data = data.frame(MSE=(c(c(MSE.evGAM[1,,]),c(MSE.gbex[1,,]),c(MSE.QNN[1,,]),c(MSE.GPDNN[1,,]))),tau=as.factor(0.95+(1-0.95)*c(rep(tau,n.exp),rep(tau,n.exp),rep(tau,n.exp),rep(tau,n.exp))),
                    label=c(rep("GAM",n.exp*length(tau)),rep("boosting",n.exp*length(tau)),rep("Q-MLP",n.exp*length(tau)),rep("GP-MLP",n.exp*length(tau))))
  p=ggplot(data, aes(x=tau, y=MSE, fill=label))+xlab(expression(tau))+ylab(expression(RMSNE(tau)))+
    geom_boxplot() + scale_y_continuous(trans='log10')+scale_x_discrete(limits=factor(0.95+(1-0.95)*tau[-c(1,6,5)]))
  ...
  p
  ggsave(p, 
         filename =paste0("RMSNE_n",ns[1],"_type",type,".pdf"),
         height = 7.5, width = 6, bg = "transparent")
  data = data.frame(MSE=(c(c(MSE.evGAM[2,,]),c(MSE.gbex[2,,]),c(MSE.QNN[2,,]),c(MSE.GPDNN[2,,]))),tau=as.factor(0.95+(1-0.95)*c(rep(tau,n.exp),rep(tau,n.exp),rep(tau,n.exp),rep(tau,n.exp))),
                    label=c(rep("GAM",n.exp*length(tau)),rep("boosting",n.exp*length(tau)),rep("Q-MLP",n.exp*length(tau)),rep("GP-MLP",n.exp*length(tau))))
  p=ggplot(data, aes(x=tau, y=MSE, fill=label)) +xlab(expression(tau))+ylab(expression(RMSNE(tau)))+ 
    geom_boxplot()+ scale_y_continuous(trans='log10')+scale_x_discrete(limits=factor(0.95+(1-0.95)*tau[-c(1,6,5)]))
  p=p+theme(legend.position="none")+ theme(text = element_text(size = 16),axis.text.x = element_text(size = 16),axis.text.y=element_text(size = 16)) 
  if(type==1) p=p+ggtitle(expression( "GP: n = " ~ 10^5)) else p=p+ggtitle(expression( "log-normal: n =" ~ 10^5))
  p
  ggsave(p, 
         filename =paste0("RMSNE_n",ns[2],"_type",type,".pdf"),
         height = 7.5, width = 6, bg = "transparent")
  
  data = data.frame(MSE=(c(c(MSE.evGAM[3,,]),c(MSE.gbex[3,,]),c(MSE.QNN[3,,]),c(MSE.GPDNN[3,,]))),tau=as.factor(0.95+(1-0.95)*c(rep(tau,n.exp),rep(tau,n.exp),rep(tau,n.exp),rep(tau,n.exp))),
                    label=c(rep("GAM",n.exp*length(tau)),rep("boosting",n.exp*length(tau)),rep("Q-MLP",n.exp*length(tau)),rep("GP-MLP",n.exp*length(tau))))
  p=ggplot(data, aes(x=tau, y=MSE, fill=label)) +xlab(expression(tau))+ylab(expression(RMSNE(tau)))+
      geom_boxplot() + scale_y_continuous(trans='log10')+scale_x_discrete(limits=factor(0.95+(1-0.95)*tau[-c(1,6,5)]))
  p=p+theme(legend.position="none")+ theme(text = element_text(size = 16),axis.text.x = element_text(size = 16),axis.text.y=element_text(size = 16)) 
  ...
  ggsave(p, 
         filename =paste0("RMSNE_n",ns[3],"_type",type,".pdf"),
         height = 7.5, width = 6, bg = "transparent")
